<?php
global $post, $intro_meta;

if ( $intro_meta ) : // Check if an intro has been selected in posts/pages/portfolios - $intro_meta = intro ID

    $page_id = ( is_home() ) ? get_option( 'page_for_posts' ) : get_the_ID();

    // Store metabox values -> Select Intro, Settings
    $intro_height = get_post_meta( $page_id, '_mokaine_intro_height', true );
    ...
    $intro_transition = get_post_meta( $page_id, '_mokaine_intro_transition', true );
    ...
    if ( $intro_transition == '' || $intro_transition == 'none' ) { 
        $intro_transition = '';
    }
    if ( $intro_navigation ) { 
    ...
    ?>
        ...
        <div id="intro-wrap" class="<?php if ( $intro_full_height ) echo 'full-height' ?>" data-height="<?php echo $intro_height ?>">

            <div id="intro" class="preload <?php if ( $intro_scroll_arrow ) echo 'more-button' ?> <?php if ( $intro_darken ) echo 'darken' ?>" data-autoplay="<?php echo $intro_autoplay ?>" data-navigation="<?php esc_attr_e( $intro_navigation ) ?>" data-pagination="<?php esc_attr_e( $intro_pagination ) ?>" data-transition="<?php esc_attr_e( $intro_transition ) ?>">
                                        ...
                                        <?php if ( $slide_subtitle ) : ?>
                                            ...
                                            <p><?php echo $slide_subtitle ?></p>
	                                            ...
	                                            <div class="row-content buffer even clear-after">
                                                              <p>  <?php  if( function_exists( 'mc4wp_form' ) ) {
    mc4wp_form();
} ?></p>
	                                            </div>
